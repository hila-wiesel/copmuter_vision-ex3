...
def displayOpticalFlow(img: np.ndarray, pts: np.ndarray, uvs: np.ndarray):
    ...
    plt.savefig("Optical Flow", dpi=300, bbox_inches='tight')
    plt.show()
...
def pyrGaussianDemo(img_path):
    ...
    plt.savefig("Gaussian Pyramid", dpi=300, bbox_inches='tight')
    plt.show()

    # gaussExpand:
    # img_level1 = gau_pyr[3]
    #
    # # gray: ****************
    # img_level1_gray = cv2.cvtColor(img_level1, cv2.COLOR_BGR2GRAY)
    # k_size = 5
    # sigma = 0.3 * ((k_size - 1) * 0.5 - 1) + 0.8
    # kernel = cv2.getGaussianKernel(k_size, sigma)
    # img_level0 = gaussExpand(img_level1, kernel)
    # plt.imshow(img_level0)
    # plt.show()
...
def pyrLaplacianDemo(img_path):
    ...
    plt.savefig("Laplacian Pyramid", dpi=300, bbox_inches='tight')
    plt.show()
...
def blendDemo():
    ...
    mask = cv2.cvtColor(cv2.imread('mask_cat.jpg'), cv2.COLOR_BGRA2RGB) / 255
    ...
    plt.title("Blending")
    plt.savefig("Blending", dpi=300, bbox_inches='tight')
    plt.show()
